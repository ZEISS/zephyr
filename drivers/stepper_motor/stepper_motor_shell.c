...
#define MOTOR_INDEX_ARGV_INDEX      (MOTOR_CONTROLLER_ARGV_INDEX + 1)
#define AUXILIARY_ARG1              (MOTOR_INDEX_ARGV_INDEX + 1)
...
int32_t zero_position_in_negative_direction(const struct stepper_motor *motor,
					    int32_t *min_pos, int32_t *max_pos)
{
	...
	stepper_motor_run(motor, &run_info);
	while (stall_status == false) {
		stepper_motor_get_stall_status(motor, &stall_status);
		k_sleep(K_MSEC(100));
	}
	stepper_motor_stop(motor);
	stepper_motor_get_position(motor, &pos_actual_info);
...
int32_t end_to_end_calibrate_function(const struct stepper_motor *motor, int32_t *min_pos,
				      int32_t *max_pos)
{
	bool stall_status = false;
...
static int cmd_stepper_motor_reset(const struct shell *shell_instance, size_t argc, char **argv)
{
	...
	stepper_motor_device = device_get_binding(argv[MOTOR_CONTROLLER_ARGV_INDEX]);
	...
	char *end_ptr;
	uint32_t index = (uint32_t)strtoul(argv[MOTOR_INDEX_ARGV_INDEX], &end_ptr, 10);
	...
	const struct stepper_motor motor = {.controller = stepper_motor_device,
						    .motor_index = index};
	shell_print(shell_instance, "resetting %s", stepper_motor_device->name);
	stepper_motor_reset(&motor);
...
static int cmd_stepper_motor_run(const struct shell *shell_instance, size_t argc, char **argv)
{
	...
	for (idx_map = 0U; idx_map < ARRAY_SIZE(stepper_motor_direction_map); idx_map++) {
		if (strcmp(argv[AUXILIARY_ARG1], stepper_motor_direction_map[idx_map].name) == 0) {
			...
			const struct stepper_motor motor = {
				.controller = stepper_motor_device, .motor_index = index};
			stepper_motor_run(&motor, &run_info);
		}
	}
...
static int cmd_stepper_motor_calibrate(const struct shell *shell_instance, size_t argc, char **argv)
{
	...
	for (idx_map = 0U; idx_map < ARRAY_SIZE(stepper_motor_calibrate_func_map); idx_map++) {
		if (strcmp(argv[AUXILIARY_ARG1], stepper_motor_calibrate_func_map[idx_map].name) ==
		    0) {
			shell_print(shell_instance, "calibrating %s", stepper_motor_device->name);
			const struct stepper_motor motor = {
				.controller = stepper_motor_device, .motor_index = index};
			stepper_motor_register_calibrate_func(
				&motor, stepper_motor_calibrate_func_map[idx_map]
					  ...
					  char **argv)
{
	...
	for (idx_map = 0U; idx_map < ARRAY_SIZE(stepper_motor_position_type_map); idx_map++) {
		if (strcmp(argv[AUXILIARY_ARG1], stepper_motor_position_type_map[idx_map].name) ==
		    0) {

			char *end_ptr;
			uint32_t position = (uint32_t)strtoul(argv[AUXILIARY_ARG2], &end_ptr, 10);
			...
			uint32_t index =
				(uint32_t)strtoul(argv[MOTOR_INDEX_ARGV_INDEX], &end_ptr, 10);
			...
			const struct stepper_motor motor = {
				.controller = stepper_motor_device, .motor_index = index};
			stepper_motor_set_position(&motor, &pos_info);
		}
	}
					  ...
					  char **argv)
{
	...
	for (idx_map = 0U; idx_map < ARRAY_SIZE(stepper_motor_position_type_map); idx_map++) {
		if (strcmp(argv[AUXILIARY_ARG1], stepper_motor_position_type_map[idx_map].name) ==
			...
			const struct stepper_motor motor = {
				.controller = stepper_motor_device, .motor_index = index};
			stepper_motor_get_position(&motor, &pos_info);
			shell_print(shell_instance, "setting %s motor %s position to %d",
				    stepper_motor_device->name,
