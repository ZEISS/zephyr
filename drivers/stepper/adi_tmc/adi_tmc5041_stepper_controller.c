...
struct tmc5041_stepper_config {
	...
	const uint32_t sg_activation_delay_in_ms;
...
static int stallguard_enable(const struct device *dev, const bool enable)
{
	...
	if (enable) {
		...
		if (abs(actual_velocity) < config->sg_threshold_velocity) {
			LOG_DBG("stallguard threshold velocity not yet reached");
			return -EAGAIN;
		}
	} else {
		reg_value &= ~TMC5041_SW_MODE_SG_STOP_ENABLE;
...
static void stallguard_work_handler(struct k_work *work)
{
	...
	if (err == -EAGAIN) {
		...
		k_work_reschedule(dwork, K_MSEC(stepper_config->sg_activation_delay_in_ms));
	}
	if (err == -EIO) {
...
static void emit_signal(struct k_poll_signal *async_signal, const enum stepper_signal_result signal)
{
	...
	if (!k_poll_signal_raise(async_signal, signal)) {
		LOG_ERR("Failed to raise signal %d", signal);
	}
}
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	if (ramp_stat_values > 0) {
		...
		case TMC5041_POS_REACHED_EVENT:
			LOG_INF("RAMPSTAT %s:Position reached", stepper_data->stepper->name);
			emit_signal(stepper_data->async_signal, STEPPER_SIGNAL_STEPS_COMPLETED);
			break;

		case TMC5041_STOP_SG_EVENT:
			LOG_INF("RAMPSTAT %s:Stall detected", stepper_data->stepper->name);
				...
				struct k_poll_signal *async)
{
	...
	if (config->sg_threshold_velocity) {
	...
	if (config->sg_threshold_velocity) {
		k_work_reschedule(&data->stallguard_dwork,
				  K_MSEC(config->sg_activation_delay_in_ms));
	}
#ifdef CONFIG_STEPPER_ADI_TMC5041_RAMPSTAT_POLL
...
static int tmc5041_stepper_init(const struct device *dev)
{
	...
	if (stepper_config->sg_threshold_velocity) {

		k_work_init_delayable(&data->stallguard_dwork, stallguard_work_handler);

		tmc5041_write(stepper_config->controller, TMC5041_SWMODE(stepper_config->index),
			      BIT(10));

		LOG_DBG("Setting stall guard to %d with delay %d ms", stepper_config->sg_threshold,
			stepper_config->sg_activation_delay_in_ms);
		...
		if (err == -EAGAIN) {
			LOG_ERR("Trying again to enable stallguard");
			k_work_reschedule(&data->stallguard_dwork,
					  K_MSEC(stepper_config->sg_activation_delay_in_ms));
		}
	}
...
#define TMC5041_STEPPER_CONFIG_DEFINE(child)							\
	BUILD_ASSERT((DT_PROP(child, stallguard_threshold_velocity) >= 0),			\
	...
	static const struct tmc5041_stepper_config tmc5041_stepper_config_##child = {		\
		...
		.stallguard_threshold = DT_PROP_OR(child, stallguard_threshold, 0),		\
		.stallguard_threshold_velocity = DT_PROP_OR(child,				\
							stallguard_threshold_velocity, 0),	\
		.stallguard_delay_in_ms = DT_PROP_OR(child, stallguard_delay_ms, 0),		\
...
#define TMC5041_DEFINE(inst)									\
	...
	static const struct tmc5041_config tmc5041_config_##inst = {				\
		.gconf = (									\
		(DT_INST_PROP_OR(inst, poscmp_enable, 0) << TMC5041_GCONF_POSCMP_ENABLE_SHIFT) |\
		(DT_INST_PROP_OR(inst, test_mode, 0) << TMC5041_GCONF_TEST_MODE_SHIFT) |	\
		(DT_INST_PROP_OR(inst, shaft1, 0) << TMC5041_GCONF_SHAFT1_SHIFT) |		\
		(DT_INST_PROP_OR(inst, shaft2, 0) << TMC5041_GCONF_SHAFT2_SHIFT) |		\
		(DT_INST_PROP_OR(inst, lock_gconf, 0) << TMC5041_LOCK_GCONF_SHIFT)),		\
