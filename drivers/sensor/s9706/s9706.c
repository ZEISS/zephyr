...
static int s9706_convert_channel_to_index(enum sensor_channel chan, enum s9706_channel *mapped_idx)
{
	switch (chan) {
	...
	case SENSOR_CHAN_BLUE:
		*mapped_idx = BLUE;
		break;
	default:
		LOG_ERR("Cannot map unsupported color channel: %d", chan);
		return -ENOTSUP;
	}
...
{
	const struct s9706_config *config = dev->config;
	...
	switch (attr) {
	case SENSOR_ATTR_S9706_CLOCK:
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		/* Alias attribute supported for easier use on the sensor shell */
		if (val->val1 <= 0) {
			LOG_ERR("S9706 clock frequency must be positive and non-zero");
			return -ENOTSUP;
		}
		if (val->val1 > MHZ(2)) {
		...
		break;
	case SENSOR_ATTR_S9706_INTEGRATION_TIME:
	case SENSOR_ATTR_BATCH_DURATION:
		/* Alias attribute supported for easier use on the sensor shell */
		if (val->val1 <= 0) {
			LOG_ERR("S9706 integration time must be positive and non-zero");
			return -ENOTSUP;
		}
		data->integration_time = val->val1;
		break;
	case SENSOR_ATTR_RESOLUTION:
		if (config->range.port == NULL) {
...
static int s9706_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	for (int i = 0; i < NUM_OF_COLOR_CHANNELS; i++) {
		...
		for (int j = 0; j < S9706_NUM_BITS; j++) {
			(void)gpio_pin_set_dt(&config->clock, 1);
			k_busy_wait(cycle_wait_us);

			(void)gpio_pin_set_dt(&config->clock, 0);
			     ...
			     struct sensor_value *val)
{
	...
	enum s9706_channel mapped_channel;
	const int ret = s9706_convert_channel_to_index(chan, &mapped_channel);

	if (ret < 0) {
		return ret;
	}

	uint32_t raw_value = data->samples[mapped_channel];
...
static int s9706_sensor_init(const struct device *dev)
{
	...
	if (config->range.port != NULL) {
		if (!gpio_is_ready_dt(&config->range)) {
			LOG_ERR_DEVICE_NOT_READY(config->range.port);
			return -ENODEV;
		}
	} else {
		LOG_DBG("S9706 RANGE GPIO not available, range configuration will be disabled");
	}

	if (!gpio_is_ready_dt(&config->gate)) {
		LOG_ERR_DEVICE_NOT_READY(config->gate.port);
		return -ENODEV;
	}

	if (!gpio_is_ready_dt(&config->data)) {
		LOG_ERR_DEVICE_NOT_READY(config->data.port);
		return -ENODEV;
	}

	if (!gpio_is_ready_dt(&config->clock)) {
		LOG_ERR_DEVICE_NOT_READY(config->clock.port);
		return -ENODEV;
	}
	...
	if (config->range.port != NULL) {
		...
		ret = gpio_pin_set_dt(&config->range, 1);
		if (ret < 0) {
			LOG_ERR("Could not set range GPIO (%d)", ret);
			return ret;
		}
		data->high_range = true;
#else  /* !CONFIG_S9706_DEFAULT_RANGE */
		ret = gpio_pin_set_dt(&config->range, 0);
		if (ret < 0) {
			LOG_ERR("Could not set range GPIO (%d)", ret);
			return ret;
		}
	...
	ret = gpio_pin_set_dt(&config->clock, 0);
	if (ret < 0) {
...
#define S9706_SENSOR_INIT(i)									\
	static struct s9706_data s9706_sensor_data_##i = {					\
		...
		.integration_time = DT_INST_PROP(i, integration_time)				\
												...
												\
	static const struct s9706_config s9706_sensor_config_##i = {				\
		.range = GPIO_DT_SPEC_INST_GET_OR(i, range_gpios, {0}),				\
